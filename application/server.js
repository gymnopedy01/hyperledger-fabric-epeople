...
app.get('/api/all-complaint-requests.json', async (req,res) => {
    ...
    try {
        ...
        // queryCar transaction - requires 1 argument, ex: ('queryCar', 'CAR4')
        // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
        const result = await contract.evaluateTransaction("ListComplaintRequestAll");
        console.log(
            `Transaction has been evaluated, result is: ${result.toString()}`
        );
...
app.patch('/api/complaint-request-status.json', async (req,res) => {
    ...
    console.log('PATCH /api/comlaint-request-status.json', requestId, userId, receptionStatus, receptionDate)
    ...
    try {
        ...
        await contract.submitTransaction(
            ...
            , requestId, receptionStatus, receptionDate
        );
...
app.post('/api/complaint-result.json', async (req,res) => {
    ...
    const resultContent = req.body.resultContent


    var uuid = v4();
    const resultId = "RES:" + uuid;

    console.log("POST /api/complaint-result.json", requestId , resultId, agency, userId, manager , resultDate ,resultContent);
    ...
    try {
        ...
        await contract.submitTransaction(
            "AddComplaintResult"
            , requestId , resultId, agency, userId, manager , resultDate ,resultContent
        );
...
app.put('/api/complaint-result.json', async (req,res) => {
    ...
    console.log("PUT /api/complaint-result.json", requestId , agency, userId, manager , resultDate ,resultContent);
    ...
    try {
        ...
        await contract.submitTransaction(
            ...
            , requestId , agency, userId, manager , resultDate ,resultContent
        );
